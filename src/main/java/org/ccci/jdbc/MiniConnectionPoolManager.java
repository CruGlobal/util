...
// slightly modified by Matt Drees - formatting and package renaming
 ...
 * 2009-06-26: Variable recycledConnections changed from Stack to Queue, so that the unused connections are reused
 * in a circular manner. Thanks to Daniel Jurado for the tip.<br>
 * 2009-08-21: ArrayDeque (which was introduced with change 2009-06-26) replaced by LinkedList, because ArrayDeque
 * is only available since Java 1.6 and we want to keep MiniConnectionPoolManager compatible with Java 1.5.<br>
 */
public class MiniConnectionPoolManager
{
    ...
    /**
     * Thrown in {@link #getConnection()} when no free connection becomes available within <code>timeout</code>
     * seconds.
     */
    ...
    /**
     * Retrieves a connection from the connection pool. If <code>maxConnections</code> connections are already in
     * use, the method waits until a connection becomes available or <code>timeout</code> seconds elapsed. When the
    ...
    public Connection getConnection() throws SQLException
    {
        ...
        try
        {
            if (!semaphore.tryAcquire(timeout, TimeUnit.SECONDS)) throw new TimeoutException();
        }
    ...
    {
        if (isDisposed) throw new IllegalStateException("Connection pool has been disposed."); // test again with
                                                                                               // lock
     ...
     * objects that have been issued by {@link #getConnection()} for which <code>Connection.close()</code> has not
     * yet been called.
     * 
